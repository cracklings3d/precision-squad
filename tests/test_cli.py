...
def test_bootstrap_blocks_non_bootstrap_skill_md_with_heuristic_phrases(
    capsys, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """Bootstrap blocks a preexisting non-bootstrap SKILL.md that contains the old heuristic phrases.

    The old heuristic detection looked for '# Precision Squad' and 'control plane' in SKILL.md.
    A manually created or install-skill-generated SKILL.md that happens to contain these phrases
    must still be blocked, not silently reused or overwritten.
    """
    monkeypatch.setattr("builtins.input", lambda prompt: "yes")
    monkeypatch.setattr(
        "precision_squad.bootstrap.check_bootstrap_prerequisites",
    ...
    # Create a SKILL.md that contains the old heuristic phrases but NOT the bootstrap marker
    (tmp_path / "SKILL.md").write_text(
        "# Precision Squad\n\nUse `precision-squad` as the control plane for issue-driven work.\n\n## control plane section\n",
        encoding="utf-8",
    )
    ...
    assert status == 1
    assert "not managed by bootstrap" in captured.err
    # Verify the unmanaged SKILL.md was NOT modified
    assert (tmp_path / "SKILL.md").read_text(encoding="utf-8") == (
        "# Precision Squad\n\nUse `precision-squad` as the control plane for issue-driven work.\n\n## control plane section\n"
    )


def test_bootstrap_blocks_install_skill_generated_skill_md(
    capsys, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """Bootstrap blocks a SKILL.md that was generated by install-skill.

    install-skill uses project_skill_template.md which does not contain the bootstrap marker.
    Bootstrap must detect this as unmanaged and block, not silently reuse/update it.
    """
    monkeypatch.setattr("builtins.input", lambda prompt: "yes")
    monkeypatch.setattr(
        "precision_squad.bootstrap.check_bootstrap_prerequisites",
    ...
    # Simulate install-skill writing a SKILL.md (from project_skill_template.md)
    from precision_squad.cli import _load_project_skill_template
    (tmp_path / "SKILL.md").write_text(_load_project_skill_template(), encoding="utf-8")
    ...
    assert status == 1
    assert "not managed by bootstrap" in captured.err
    # Verify the install-skill SKILL.md was NOT modified
    assert (tmp_path / "SKILL.md").read_text(encoding="utf-8") == _load_project_skill_template()


def test_bootstrap_allows_bootstrap_owned_skill_md_reuse(
    capsys, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """Bootstrap allows SKILL.md that was previously written by bootstrap (has the bootstrap marker)."""
    monkeypatch.setattr(
        "precision_squad.bootstrap.check_bootstrap_prerequisites",
        lambda project_root: None,
    )

    # First, run bootstrap to create the SKILL.md
    status1 = bootstrap_main(["--project-root", str(tmp_path), "--yes"])
    assert status1 == 0

    # Verify SKILL.md has the bootstrap marker
    skill_content = (tmp_path / "SKILL.md").read_text(encoding="utf-8")
    assert "<!-- precision-squad bootstrap managed skill -->" in skill_content

    # Run bootstrap again - should succeed and report already_satisfied
    status2 = bootstrap_main(["--project-root", str(tmp_path), "--yes"])
    assert status2 == 0
...
def test_run_issue_fresh_without_approved_plan_path_reaches_coordinator(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    intake = IssueIntake(
        issue=GitHubIssue(
    ...
    def fake_repair_issue(self, *, params, intake, dependencies):
        del self, dependencies
        captured["approved_plan"] = params.approved_plan
        captured["review_stages"] = params.review_stages
        return RepairIssueReport(
            intake=intake,
            run_record=RunRecord(
                run_id="run-1",
    ...
    assert status == 0
    assert captured["approved_plan"] is None
    assert captured["review_stages"] is None


def test_run_issue_explicit_fresh_without_approved_plan_path_loads_intake_and_reaches_coordinator(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    intake = IssueIntake(
        issue=GitHubIssue(
    ...
    def fake_repair_issue(self, *, params, intake, dependencies):
        ...
        captured["intake_ref"] = canonicalize_local_issue_ref(str(intake.issue.reference))
        return RepairIssueReport(
            intake=intake,
            run_record=RunRecord(
                run_id="run-1",
    ...
    assert captured["intake_ref"] == "owner/repo#1"
...
def test_run_issue_prompt_selected_fresh_without_plan_reaches_coordinator(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    runs_dir = tmp_path / "runs"
    store = RunStore(runs_dir)
...
def test_repair_issue_help_mentions_compatibility_ingress_and_review_stages(capsys) -> None:
    with pytest.raises(SystemExit) as exc_info:
        main(["repair", "issue", "--help"])

    captured = capsys.readouterr()
    assert exc_info.value.code == 0
    assert "compatibility ingress" in captured.out
    assert "retries may omit it" in captured.out
    assert "--review-stages {plan,all}" in captured.out


@pytest.mark.parametrize("review_stages", ["plan", "all"])
def test_repair_issue_review_stages_values_forward_into_params(
    review_stages: str, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    intake = IssueIntake(
        issue=GitHubIssue(
            reference=IssueReference("owner", "repo", 1),
    ...
    assert status == 0
    assert captured["review_stages"] == review_stages


def test_repair_issue_review_stages_rejects_unsupported_value(capsys, tmp_path: Path) -> None:
    with pytest.raises(SystemExit) as exc_info:
        main(
            [
                "repair",
